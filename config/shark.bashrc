########################
# READLOG + GREPLOG
########################

########################
# HISTORY
########################

export HISTFILESIZE=10000
export HISTSIZE=500
export HISTTIMEFORMAT="%F %T"
export HISTCONTROL=erasedups:ignoredups:ignorespace
shopt -s histappend

########################
# COLOR
########################
export TERM=xterm-256color
# 0  - No style         1  - Bold             2  - Dim                  3  - Italic       4  - Underline        
# 5  - Slow blink       6  - Rapid blink      7  - Reverse (fg-bg)      8  - Hidden       9  - Strikethrough
__hex_to_ansi() {
  local hex="${1#\#}" 
  local style="${2:-0}" # 
  local r=$((16#${hex:0:2}))
  local g=$((16#${hex:2:2}))
  local b=$((16#${hex:4:2}))
  printf '\e[%s;38;2;%d;%d;%dm' "$style" "$r" "$g" "$b"
}

########################
# PROMPT
########################
PROMPT_COMMAND=__prompt_command 

__color() {
    local color="$1" 
    local text="$2"

    local col_0='\[\e[0m\]' 
    local col_ylw="\[$(__hex_to_ansi EFBD58 2)\]"
    local col_grn="\[$(__hex_to_ansi A5CD84 1)\]"
    local col_red="\[$(__hex_to_ansi F8747E 1)\]"
    local col_blu="\[$(__hex_to_ansi 8DC3F1 1)\]"
    local col_pur="\[$(__hex_to_ansi DEAEED 1)\]"

    local col_var="col_$color"

    printf "%b%s%b" "${!col_var}" "$text" "$col_0"
}

__trimmed_pwd() {
  local dir="${PWD/#$HOME/~}"
  local icon_flask="$(printf "\xF3\xB0\x82\x93")"
  local icon_beach="$(printf "\xEF\x94\xA2")"

  if [[ $dir =~ ^/tmp/data/([^/]+)/data(/.*)?$ ]]; then 
    local base="${BASH_REMATCH[1]}"
    local rest="${BASH_REMATCH[2]}"
    [[ -z $rest ]] && rest=""
    dir="$icon_flask${base}${rest}"
  fi

  dir="${dir/\/tmp\/local\//$icon_beach/}"

  IFS='/' read -r -a parts <<< "$dir"
  local count=${#parts[@]}

  if (( count > 4 )); then
    printf ".../%s/%s\n" "${parts[count-2]}" "${parts[count-1]}"
  else
    printf "%s\n" "$dir"
  fi
}

__git_prompt() {
    local icon_fish="$(printf "\xEE\xB9\x81")"
    local icon_water="$(printf "\xF3\xB0\x96\x8C")"
    local icon_ticket="$(printf "\xF3\xB0\x94\x98")"


    if ! git rev-parse --is-inside-work-tree &>/dev/null; then
        return
    fi

    local branch=$(git symbolic-ref --short HEAD 2>/dev/null || git rev-parse --short HEAD 2>/dev/null)
    if [[ "$branch" == *.*.*.* ]]; then
        branch=$icon_ticket$(echo "$branch" | cut -d'.' -f3)
    fi

    local cherry
    cherry=$(git cherry -v 2>/dev/null)

    local unpushed_commit
    unpushed_commit=$(grep -c '^+' <<< "$cherry")

    local missing_commit
    missing_commit=$(grep -c '^-' <<< "$cherry")

    local commit_flag=""
    if (( unpushed_commit > 0 )); then
        if (( unpushed_commit > 1 )); then
            commit_flag=" $(__color grn $unpushed_commit$icon_fish)"
        else
            commit_flag=" $(__color grn $icon_fish)"
        fi
    fi
    
    local missing_flag=""
    if (( missing_commit > 0 )); then
        missing_flag="$(__color blu $icon_water) "
    fi

    echo "(${missing_flag}${branch}${commit_flag}) "
}

__env_prompt () {
    local icon_fishtank
    icon_fishtank="$(printf '\xF3\xB0\xBB\xB3')"

    if [[ -z "$VAR" ]]; then
        return
    fi

    echo "$(__color grn "[$VAR $icon_fishtank]") "
}

__prompt_command() {
    local ec=$?

    local arrow="><> "
    if [[ $ec -ne 0 ]]; then
        arrow="$(__color red "$arrow")"
    else
        arrow="$(__color blu "$arrow")"
    fi

    local prompt_user="$(__color pur "\u ")"
    local prompt_pwd="$(__color ylw "$(__trimmed_pwd) ")"
    local prompt_git="$(__git_prompt)"
    local prompt_env="$(__env_prompt)"

    PS1="${arrow}${prompt_pwd}${prompt_user}${prompt_env}${prompt_git}"
}

